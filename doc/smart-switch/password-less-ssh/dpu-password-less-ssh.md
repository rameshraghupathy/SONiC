...
o	For this we need to access the DPU first time using the default credentials
o	Don’t hard code the default credentials, provide them from a file in the platform directory ( This makes it flexible for every vendor )
o	Also, this enables the script to be independent of the default credentials
o	Encrypt the default credentials
o	All the above will be accomplished with a script and will be activated from the “config chassis modules enable_passwordless_ssh” CLI.  More details can be found in the following sections.
4.	Connect to DPUs using ssh without password
o	ssh admin@169.254.200.1  (no password needed)
...
```
•	New CLI on NPU:
o	config chassis modules enable_passwordless_ssh  --username <optional> --password <optional>
```

o	When the credentials are not provided through the CLI the credential file on the NPU “/usr/share/sonic/device/<platform>/dpupassword” will be used.  If this file doesn’t exist then the passwordless_ssh will not be enabled

o	When the credentials are provided there will be two options either to overwrite the existing file or the append to the existing file.  Then this file will be used for copying the keys the first time

o	config chassis modules disable_passwordless_ssh  DPUx  is another CLI used the disable the password. This CLI will wipeout the known hosts file on the specified DPUx

o	config chassis modules disable_passwordless_ssh  all .  This will wipeout the known hosts file on all DPUs and also remove the credential file.

o	show chassis modules passwordless_dpu_state is a show CLI which will show the passwordless_ssh_state of all the DPUs
